...
const WorkItemWrapper = ({children}) => {
    ...
    const inner = css({width: '100%', height: '100%', background: theme.colors.mono200});
    return (
        <li className={c}>
            <div className={inner}>
                {children}</div>
...
const WorkItemsWrapper = ({children}) => {
    ...
    const c = css({background: 'white', listStyleType: 'none', margin: 0, padding: 0});
...
const WorkItemLink = ({node, children}) => {
    ...
    return (
        <Link className={c}
            to={
                join(templateKeyMap[node.templateKey], node.slug.current)
        }>
            {children} </Link>
    );
};
